import { arrayCopy, byteArrayToIntArray, longToByte, intArrayToByteArray } from './utils';

class flk_SM3Digest {
  constructor() {
    this.ivByte = [
      0x73, 0x80, 0x16, 0x6f, 0x49, 0x14, 0xb2, 0xb9, 0x17, 0x24, 0x42, 0xd7, 0xda, 0x8a, 0x06,
      0x00, 0xa9, 0x6f, 0x30, 0xbc, 0x16, 0x31, 0x38, 0xaa, 0xe3, 0x8d, 0xee, 0x4d, 0xb0, 0xfb,
      0x0e, 0x4e,
    ];
    this.iv = byteArrayToIntArray(this.ivByte);
    this.tj = new Array(64);
    this.BLOCK_BYTE_LEN = 64;
    this.vbuf = new Array(8); //数据缓冲区

    this.dataBuf = new Array(64); //缓冲区长度

    this.dataBufLen = 0; //缓冲区总长度
  ...
  expand(b) {
    ...
    for (var i = 16; i < w1.length; i++) {
      w1[i] =
        this.p1(w1[i - 16] ^ w1[i - 9] ^ this.cycleLeft(w1[i - 3], 15)) ^
        this.cycleLeft(w1[i - 13], 7) ^
        w1[i - 6];
    }
  ...
  update(data, pos, len) {
    ...
    if (len + this.dataBufLen < this.BLOCK_BYTE_LEN) {
      ...
      arrayCopy(
        data,
        loop * this.BLOCK_BYTE_LEN - this.dataBufLen,
        this.dataBuf,
        0,
        len - (loop * this.BLOCK_BYTE_LEN - this.dataBufLen),
      );
  ...
  doFinal() {
    ...
    arrayCopy(this.dataBuf, 0, finalData, 0, this.dataBufLen); //对不足64字节的数据进行填充
